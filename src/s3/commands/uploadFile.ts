...
export async function uploadFileCommand(
    ...
    try {
        ...
        window.message.showError(localize('AWS.s3.uploadFile.error.general', 'Failed to upload file {0}', fileName))
...
async function promptForFileLocation(window: Window): Promise<vscode.Uri | undefined> {
    const fileLocations = await window.dialog.showOpen({
